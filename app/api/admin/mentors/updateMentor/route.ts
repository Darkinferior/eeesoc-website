...
export async function PATCH(request: Request): Promise<NextResponse> {
  try {
    ...
    if (newAreasOfInterest)
      formattedAreasOfInterest = newAreasOfInterest.split(",");
    ...
    if (existingMentor) {
      ...
      if (formattedAreasOfInterest)
        existingMentor.areasOfInterest = formattedAreasOfInterest;

      await existingMentor.save();

      return NextResponse.json({
        msg: "workshop data updated successfully",
        success: true,
      });
    } else {
